...
async function createProject(options = {}) {
  ...
  if (
    ...
    await project._addSourceFilesForTsConfigResolver(
      tsConfigResolver,
...
function createProjectSync(options = {}) {
  ...
  if (
    ...
    project._addSourceFilesForTsConfigResolverSync(
      tsConfigResolver,
      project.compilerOptions.get(),
    );
    if (!options.skipFileDependencyResolution) {
...
class Project {
  constructor(objs, options) {
    ...
    const { languageServiceHost, compilerHost } = createHosts({
      ...
      getProjectVersion: () =>
        this._sourceFileCache.getProjectVersion().toString(),
  ...
  addSourceFileAtPathIfExists(filePath, options) {
    return this._sourceFileCache.addOrGetSourceFileFromFilePath(
      this._fileSystemWrapper.getStandardizedAbsolutePath(filePath),
      {
  ...
  addSourceFileAtPathIfExistsSync(filePath, options) {
    return this._sourceFileCache.addOrGetSourceFileFromFilePathSync(
      this._fileSystemWrapper.getStandardizedAbsolutePath(filePath),
      {
        scriptKind: options && options.scriptKind,
      },
  ...
  _getTsConfigResolover(tsConfigFilePath) {
    const standardizedFilePath = this._fileSystemWrapper
  ...
  removeSourceFile(filePathOrSourceFile) {
    this._sourceFileCache.removeSourceFile(
      this._fileSystemWrapper.getStandardizedAbsolutePath(
  ...
  async _addSourceFilesForTsConfigResolver(tsConfigResolver, compilerOptions) {
    ...
    await Promise.all(
      tsConfigResolver.getPaths(compilerOptions).filePaths
        .map((p) =>
          this.addSourceFileAtPath(p).then((s) => sourceFiles.push(s))
        ),
    );
    return sourceFiles;
  }
  _addSourceFilesForTsConfigResolverSync(tsConfigResolver, compilerOptions) {
    return tsConfigResolver.getPaths(compilerOptions).filePaths.map((p) =>
      this.addSourceFileAtPathSync(p)
    );
  }
  ...
  getLanguageService() {
    return ts.createLanguageService(
      ...
      this._sourceFileCache.documentRegistry,
    );
  }
  getSourceFileOrThrow(fileNameOrSearchFunction) {
    ...
    if (typeof fileNameOrSearchFunction === "string") {
      ...
      if (
        ...
        const errorFileNameOrPath = this._fileSystemWrapper
  ...
  getSourceFile(fileNameOrSearchFunction) {
    ...
    if (isStandardizedFilePath(filePathOrSearchFunction)) {
      return this._sourceFileCache.getSourceFileFromCacheFromFilePath(
